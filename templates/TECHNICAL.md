...
| rank | `src/rank.ts` | Assign an importance weight per event; guarantee every goal; fill remaining slots by weight; collapse foul/penalty mirror pairs; cap each non-goal type for variety. |
...
| validate | `src/validate.ts` | Ajv (draft 2020-12) schema validation plus invariants the schema can't express. |
...
missing required key or an unknown one. The builder validates against a
corrected copy, `schema/story.fixed.schema.json`, which requires `story_id` to
match both the README and the defined property. The original is kept in the repo
...
fill-by-weight, then re-sorted chronologically, with two refinements: mirror
pairs from a single incident (foul won/lost, penalty won/lost) are collapsed,
and each non-goal type is capped (default 2) so the story stays varied. Full
rationale and the industry comparison (Opta xG / "big chance", Apple Sports Key
Plays) are in `DECISIONS.md`.
...
`npm test` runs 16 Vitest cases covering: envelope parsing, chronological
